...
function Page1() {
    ...
    const sendAudioToAPI = async (audioBlob) => {
        try {
            ...
            // ตรวจสอบว่ามีจำนวนคำไม่เกิน 4 คำหรือไม่
            if (words.length <= 4) {
                ...
                let userIdString = '';
                if (convertedNumbers.length === 4) {
                    userIdString = convertedNumbers.join('');
                } else {
                    userIdString = -1;
                }
                ...
                // ถ้าข้อความมีคำมากกว่า 4 คำ คืนค่า -1
                setUserId(-1);
                console.error('Error: More than 4 words in the transcription');
            }
        } catch (error) {
