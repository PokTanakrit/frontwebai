...
function Pagechoose6() {
    ...
    return (
        <div>
            <header>มีอาการแพ้ยาอะไรบ้าง?</header>

            <div className="input-container">
            <input type="text" value={otherSymptom} onChange={(event) => handleInputChange(event.target.value)} />
            </div>
            <div className="button-container">
                <button onClick={() => handleSymptomSelect("เคยแพ้ยา")} className={selectedSymptoms.has("เคยแพ้ยา") ? "selected" : ""}>เคยแพ้ยา</button>
                <span className="button-gap"></span>
                <button onClick={() => handleSymptomSelect("ไม่เคยแพ้ยา")} className={selectedSymptoms.has("ไม่เคยแพ้ยา") ? "selected" : ""}>ไม่เคยแพ้ยา</button>
            ...
            <div className="button-container">
                <React.Fragment>
                    ...
                    <span className="button-gap"></span>
                    <span className="button-gap"></span>
                    {selectedSymptoms.size > 0 && <Link to="/page7"><button>ถัดไป</button></Link>}
                </React.Fragment>
            </div>
        </div>
    );
